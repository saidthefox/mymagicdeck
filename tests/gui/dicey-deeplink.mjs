...
// In guide mode there is NO greeting at any point — instead he ducks UNDER the program he opened (z-index→1).
...
await p.waitForTimeout(8500); // past render-delayed folder route + settle + tuck-under
const end = await p.evaluate(()=>{ const e=(typeof _diceyEl!=='undefined')&&_diceyEl; const rc=e&&e.getBoundingClientRect();
  return { dicey:!!e, deckos:document.body.classList.contains('deckos'), openedLandgame:!!document.querySelector('.lg-start, .lg-wrap'),
    pos:rc?{x:rc.left,y:rc.top}:null, introUp:!!document.querySelector('.dicey-bubble, .dicey-say'), ducked: !!e && e.style.zIndex==='1' }; });
...
const okDesktop = end.dicey && end.deckos && end.openedLandgame && moved && !mid.introUp && !end.introUp && end.ducked && !errs.length;
console.log('moved:',moved,'introHiddenThroughout:',!mid.introUp&&!end.introUp,'duckedUnder:',end.ducked,'RESULT:', okDesktop?'PASS':'FAIL');
...
const mEnd = await mp.evaluate(()=>({ basics:!!document.querySelector('.lg-start, .lg-wrap'), introUp:!!document.querySelector('.dicey-bubble') }));
const okMobile = wasMin && curCount>=1 && mEnd.basics && !mEnd.introUp && !merrs.length;
console.log('mobile → minimizedMMD:',wasMin,'cursorPresses:',curCount,'basicsOpen:',mEnd.basics,'noIntro:',!mEnd.introUp,'errs:',merrs.length,'RESULT:', okMobile?'PASS':'FAIL');
